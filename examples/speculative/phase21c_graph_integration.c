// PRT Phase 21C: graph integration test for GGML_OP_PRT_FFN_UP
// Tests that ggml_prt_ffn_up works in a ggml_compute graph context
// (simulating the llama graph pipeline without full model)
...
// Compute expected using the SAME formula as the kernel
static void compute_expected(float * expected,
                            const float * x_vals, const float * w_vals, const float * s_vals,
                            int K, int M, int N) {
    for (int n = 0; n < N; n++) {
        for (int j = 0; j < M; j++) {
            float acc = 0.0f;
...
static int run_graph_test(int K, int M, int N,
    ...
    // Compute expected
    float expected[M * N];
    compute_expected(expected, x_vals, w_vals, s_vals, K, M, N);
    ...
    // Call ggml_prt_ffn_up (the GGML_OP_PRT_FFN_UP constructor)
...
int main(void) {
    printf("=== PRT Phase 21C Graph Integration Test ===\n");
    printf("Testing GGML_OP_PRT_FFN_UP in ggml_compute graph pipeline\n\n");

    int n_pass = 0;
    int n_total = 0;

    // Test 1: K=16 M=32 N=1
    {
        n_total++;
    ...
    // Test 3: K=8 M=4 N=1 (same as 21B-R unit test)
    {
        n_total++;
    ...
    // Test 4: K=8 M=4 N=2 (same as 21B-R unit test)
    {
        n_total++;
