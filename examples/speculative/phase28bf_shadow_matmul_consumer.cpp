...
//
// Proves the runtime-adjacent shadow consumer can:
...
// (prt_sidecar_runtime_link.h declares these extern — provide strong defs here)
...
static TritHeaderFields parse_trit_header(const uint8_t* hdr, size_t size) {
    ...
    // 0x54524954 = "TRIT", 0x54495254 = "TIRT"
...
static TestResult test_positive(const std::string& pkg_dir,
    ...
    // Step 7: Get pager-backed view via prt_get_residual_view
...
// ── Manual JSON writer (no nlohmann dependency) ─────────────────────────
static void write_json(const std::string& path,
    ...
    fprintf(f, "{\n");
    fprintf(f, "  \"phase\": \"28BF\",\n");
    fprintf(f, "  \"name\": \"Shadow Matmul Consumer End-to-End Dry Run\",\n");
    fprintf(f, "  \"branch\": \"experimental/prt-phase19a-alt-sidecar-backed\",\n");
    fprintf(f, "  \"head_before\": \"e9cc6bd2b\",\n");
    fprintf(f, "  \"verdict\": \"%s\",\n", all_pass ? "PASS" : "FAIL");
    fprintf(f, "  \"generation_run\": false,\n");
    fprintf(f, "  \"large_files_staged\": false,\n");
    fprintf(f, "  \"positive_tests\": [\n");

    const char* case_names[] = {"row_split", "col_split", "row_col_split", "awkward_edge"};
    const char* case_pkgs[] = {
        "/tmp/phase28bd_pkg_row_split",
        "/tmp/phase28bd_pkg_col_split",
        "/tmp/phase28bd_pkg_row_col_split",
        "/tmp/phase28bd_pkg_awkward_edge"
    };
...
// ── Main ──────────────────────────────────────────────────────────────────
int main() {
    printf("Phase 28BF: Shadow Matmul Consumer End-to-End Dry Run\n");
    printf("======================================================\n\n");
    ...
    Case cases[] = {
        {"row_split",     "/tmp/phase28bd_pkg_row_split",     96,  48, 4, 32, 48},
        {"col_split",     "/tmp/phase28bd_pkg_col_split",     32, 144, 4, 32, 48},
        {"row_col_split", "/tmp/phase28bd_pkg_row_col_split", 96, 144, 6, 32, 48},
        {"awkward_edge",  "/tmp/phase28bd_pkg_awkward_edge",   70, 101, 2, 32, 48},
    };

    std::vector<TestResult> pos_results;
    for (auto& c : cases) {
        TestResult tr = test_positive(c.pkg, c.name, c.K, c.M, c.N, c.br, c.bc);
    ...
    printf("Verdict: %s\n", all_pass
        ? "PASS_PHASE28BF_SHADOW_MATMUL_CONSUMER_DRY_RUN"
        : "FAIL_PHASE28BF");

    // Write JSON
    std::string json_path = "/home/matthew-villnave/llama.cpp/examples/speculative/results/phase28bf_shadow_matmul_consumer_dry_run.json";
...
int main() {
    ...
    printf("Result: FAIL_PHASE28BF (stub)\n");
    return 1;
}
